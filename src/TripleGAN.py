...
class TripleGAN(object) :
    ...
    def generator(self, z, y, scope='generator', is_training=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse) :
            ...
            s = self.output_height #Final length of the sequence

            #We want to slowly upscale the sequence, so these values should help
            # to make that change gradual
            s2, s4, s8, s16 = int(s/2), int(s/4), int(s/8), int(s/16)

            width = self.output_width #because we have 4 letters: ATCG
            magic_number = 5 #this is a magic number which I'm not sure what means yet

            h0 = tf.reshape(z, [batch_size, int(width/4), s16+1, magic_number])
            h0 = tf.nn.relu(h0)
            ...
            #Fourth DeConv Layer
            output4_shape = [batch_size, width, s, self.c_dim]
